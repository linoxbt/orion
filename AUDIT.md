...
| M2 | High | `escalate_to_human` could deliver nothing: no `SENDGRID_API_KEY`, no `TWILIO_WHATSAPP_FROM` | **Closed.** Three channels now, tried in order: SMS from the Twilio number that already places the calls - no new account, no session window - then WhatsApp on Twilio's free sandbox, then email through Resend (3,000/month free) in place of SendGrid, whose free tier ended in May 2025. `test_escalation_channels.py`. The only one still needing a key is Resend, and email is the third choice of three. |
